...
def CostosItem():
    ...
    Tax = f"{(PriceofNonveg + PriceofVeg + 12)*0.18} Rs"
    PaidTax.set(Tax)

    TT = (PriceofNonveg + PriceofVeg + 12)*0.18
    TC = PriceofNonveg + PriceofVeg + 12 + TT

    TotalCost.set(TC - varDis)
...
lblname = Label(Tops, font="arial 15 bold",
                text="By Nilesh Rangari", bg="Cadet blue", fg="Cornsilk")
...
# Discount
lblDiscount = Label(Cost_F, font="arial 14 bold",
                     text="Discount", bd=7, bg="powder blue", fg="black")
lblDiscount.grid(row=3, column=0, sticky=W)
txtDiscount = Entry(Cost_F, font="arial 12 bold", bd=7,
                     textvariable=discount, bg="white", insertwidth=2, justify=RIGHT)
txtDiscount.grid(row=3, column=1 )
